...
defineEmits<{ select: []; step: [delta: number]; edit: []; remove: [] }>();
...
<template>
  <div
    class="grid items-center gap-12px grid-cols-[minmax(0,1fr)_220px_150px_44px] px-14px py-12px rounded-16px cursor-pointer border-2 transition-colors"
    :class="active ? 'bg-[#fbeede] border-terracotta' : 'bg-line border-[#ece3d2]'"
    @click="$emit('select')"
  >
    <!-- ITEM -->
